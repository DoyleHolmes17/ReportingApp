 ...
 */
public class UpdatePenugasanTeknisiAsyncTask extends AsyncTask<Object, Integer, ListLaporan> {

    private Context context;
    private String header;
    private HashMap<String, String> hashMap;

    public UpdatePenugasanTeknisiAsyncTask(Context context, HashMap<String, String> hashMap, String header) {
    ...
    protected ListLaporan doInBackground(Object... objects) {
        ...
        String url = Urls.urllaporan + Urls.updatepenugasanteknisi;
        ...
        try {
            if (result != null) {
                ...
                if (data != null) {
                    ...
                    String is_userteknis = jsonObject.has(FieldName.ID_USERTEKNIS) ? jsonObject.getString(FieldName.ID_USERTEKNIS) : "";

                    laporan = new ListLaporan();
                    laporan.setStatus(status);
                    laporan.setMessage(message);
                    laporan.setNote_admin(note_admin);
                    laporan.setId_userteknis(is_userteknis);
                } else {
                    laporan = new ListLaporan();
                    laporan.setStatus(status);
                    laporan.setMessage(message);
